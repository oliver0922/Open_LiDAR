...
def text_extractor(normalize_feature=True): 
    model, preprocess = clip.load("ViT-L/14@336px", mask_prompt_depth=0, device=device)
...
def main(args):
    ...
    lidar_paths = sorted(glob(os.path.join(segment_name, 'lidar', '*.bin')))
    ...
    for text_idx, text_feature in enumerate(text_features):
        ...
        os.makedirs(f'./{segment_name}/output/{NUSCENES_PREDEFINED[text_idx]}', exist_ok=True)
        for lidar_path in tqdm(lidar_paths):
            ...
            for cam_id, camera_name in enumerate(camera_names):
                intr = np.load(os.path.join(segment_name, 'K', f'{camera_name}.npy'))
                pose = np.loadtxt(os.path.join(segment_name, 'pose', f'{scene_name}_{camera_name}.txt'))
                lidar_pose = np.loadtxt(os.path.join(segment_name, 'lidar_pose', f'{scene_name}.txt'))
                ...
                clip_path = os.path.join(segment_name, 'clip', f'{scene_name}_{camera_name}.pt')
                mask_path = os.path.join(segment_name, 'mask', f'{scene_name}_{camera_name}.pt')
                clip_features = torch.load(clip_path).detach().cpu().numpy()
                masks = torch.load(mask_path).detach().cpu().numpy()
            ...
            o3d.io.write_point_cloud(f'./{segment_name}/output/{NUSCENES_PREDEFINED[text_idx]}/{scene_name}.pcd', pcd)
            # import pdb; pdb.set_trace()
            

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--segment', type=str, default='scene-1060')

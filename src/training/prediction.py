...
class Prediction():
    def __init__(self, text_set, model, device, dim=768):
    ...
    def predict(self, metadata):
        points, clip_features, mask, poses, label = metadata['coords'], metadata['clip'], metadata['mask'], metadata['poses'], metadata['label']


        {"coords":torch.from_numpy(point), 
                "colors_path": color_image_paths,
                "coords_path": lidar_path,
                "feats": torch.ones_like(torch.from_numpy(label)),

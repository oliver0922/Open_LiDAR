...
def prediction_model(config, data, text_feats, model, device):
    ...
    distill_loss = (1 - torch.nn.CosineSimilarity()(out_feats[masks], clip_feats)).mean()
...
def val(config, model, epoch, val_dataloader, device, dataset, sanity=False):
    pbar = tqdm(val_dataloader, desc = "Validation")

    if dataset == 'nuscenes':
        mapping = config.text_to_nuscene
    elif dataset == 'waymo':
        mapping = config.text_to_waymo
    ...
    for idx, data in enumerate(pbar):
        with torch.no_grad():
            ...
            clip_pred.append(mapping[torch.argmax(clip_prediction,dim=1)])

            distill_pred.append(mapping[torch.argmax(distill_prediction,dim=1)])

            ensemble_pred.append(mapping[torch.argmax(ensemble_prediction,dim=1)])
    ...
    clip_iou = evaluate(clip_pred.cpu().numpy(), labels_masked.cpu().numpy(), dataset=dataset)
    print('----------------------------------------')
    print('Distillation CLIP feature results')
    distill_iou = evaluate(distill_pred.cpu().numpy(), labels_.cpu().numpy(), dataset=dataset)
    print('----------------------------------------')
    print('Ensenble CLIP feature results')
    ensemble_iou = evaluate(ensemble_pred.cpu().numpy(), labels_.cpu().numpy(), dataset=dataset)
    print('----------------------------------------')
...
def main(config):
    ...
    train_dataloader, val_dataloader = get_mink_generator(config.nuScene_path, config.batch_size, config.workers, dataset='nuscenes', multisweep=True)

    train_dataloader_waymo, val_dataloader_waymo = get_mink_generator(config.Waymo_path, config.batch_size, config.workers, dataset='waymo')
    ...
    text_to_waymo = torch.tensor([11, 11, 12, 3, 1, 4, 4, 4, 4, 4, 13, 7, 7, 11, 4, 4, 4, 4, 4, 2, 18, 17, 20, 20, 
    ...
    config.text_to_waymo = text_to_waymo
    ...
    for epoch in range(start_epoch, config.max_epochs):
        if epoch != 0:
            ...
            if epoch % config.val_duration == 0:
                _,_,val_nuscene_results = val(config, models, epoch, val_dataloader, device, dataset='nuscenes', sanity=False)
                _,_,val_waymo_results = val(config, models, epoch, val_dataloader_waymo, device, dataset='waymo', sanity=False)

        else: 
            _,_,val_nuscene_results = val(config, models, epoch, val_dataloader, device, dataset='nuscenes', sanity=False)
            _,_,val_waymo_results = val(config, models, epoch, val_dataloader_waymo, device, dataset='waymo', sanity=False)

        if val_miou_max < val_waymo_results : 
            is_best = True
            val_miou_max = val_waymo_results

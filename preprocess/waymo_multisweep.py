...
def process_one_sequence(args, split, scene_path, scene_name):
    ...
    for data in dataset:
        ...
        if frame.lasers[0].ri_return1.range_image_compressed:
            (range_images, camera_projections, segmentation_labels, range_image_top_pose) = \
                frame_utils.parse_range_image_and_camera_projection(frame)
            
            if points_agg is None and prev_lidar_pose is not None:
                raise AssertionError('points_agg is None but prev_lidar_pose is not None')
            if points_agg is not None and prev_lidar_pose is None:
                raise AssertionError('points_agg is not None but prev_lidar_pose is None')
            ...
            if points_agg is not None:
                rel_pose = np.linalg.inv(prev_lidar_pose) @ lidar_pose
                points_agg = np.hstack((points_agg, np.ones((points_agg.shape[0], 1))))
                tmp = np.linalg.inv(rel_pose) @ points_agg.T
                points_agg = tmp.T[:, :3]

            # No segmentation labels
            if segmentation_labels == {}:
                points_agg = np.concatenate((points_agg, points_all)) if points_agg is not None else points_all
                prev_lidar_pose = lidar_pose

            else:
                # LiDAR points and 3D labels
                if 'multi_lidar' in args.targets or 'label' in args.targets or 'multi_lidar_mask' in args.targets:
                    ...
                    points_agg = np.concatenate((points_agg[:, :3], points_labels_concat_filtered[:, :3])) if points_agg is not None else points_labels_concat_filtered[:, :3]

                    points_mask = np.zeros_like(points_agg)
                    points_mask[-points_labels_concat_filtered.shape[0]:] = 1

                    if 'multi_lidar' in args.targets:
                        points_agg.tofile(os.path.join(out_dirs['multi_lidar'], f'{str(frame.timestamp_micros)}.bin'))
                    if 'multi_lidar_mask' in args.targets:
                        points_mask.tofile(os.path.join(out_dirs['multi_lidar_mask'], f'{str(frame.timestamp_micros)}.bin'))
                ...
                if 'color' in args.targets or 'pose' in args.targets:
                    for idx, image in enumerate(frame.images):
                        ...
                        if 'color' in args.targets:
                            ...
                            image_jpeg = np.asarray(tf.image.resize(image_jpeg, (450, 800))).astype(np.uint8)
...
def main(args):
    ...
    if not set(args.targets) <= set(['color', 'multi_lidar', 'multi_lidar_mask', 'label', 'lidar_pose', 'pose', 'K', 'bbox']):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--targets', type=str, nargs='+', default=['color', 'multi_lidar', 'multi_lidar_mask', 'label', 'lidar_pose', 'pose', 'K', 'bbox'])
